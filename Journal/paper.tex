...
Larder is a cross-platform intelligent recipe app for everyday cooking on Android and iOS. It targets users with fast-paced lifestyles who need to decide what to cook quickly. Recipe content is fragmented across websites, short-form videos, and social media, making it difficult to save, organize, and reuse recipes in one place, and to turn them into actionable meal plans and shopping lists.

User research revealed four recurring issues across the cooking workflow. First, as users save more recipes, it becomes harder to quickly find options that fit their tastes, time constraints, and available ingredients. Second, unstructured recipe media such as videos is difficult to follow while cooking and often requires repeated pausing and scrubbing. Third, meal planning and grocery decisions are often disconnected. When planning meals for the next few days, users must juggle constraints such as budget, health goals, dietary restrictions, and variety. After a plan is set, they still need to decide what to buy, how much to buy, and how to avoid duplicates given what is already in the fridge. Finally, in-kitchen contexts make touch interaction inconvenient with wet or greasy hands, yet users still need smooth step-to-step navigation.

Based on these issues, we designed an end-to-end journey from discovery to planning, shopping, and in-kitchen execution. For structured planners, users add dishes to Meal Plan and the app generates a Grocery List for upcoming meals. For more ad hoc cooking, users can stock ingredients and scan the fridge right before cooking to trigger recommendations. A conversational AI assistant supports personalized requests, for example lighter high-protein meals, 30-minute options, or avoiding repeat purchases.

At the feature level, Discover helps users quickly find what to cook from overwhelming content using personalized recommendations based on user preferences and in-app behaviour (for example, searches, clicks, saves, and ratings). Larder also provides recipe management, user accounts, Meal Plan, and Grocery, so users can move from discovering a dish to scheduling it and generating a shopping list within one app. Recipes can be imported from external sources such as URLs, images, or videos for unified management.

To support cooking execution, we aim to make the in-kitchen experience convenient when users' hands are busy. Our step-by-step guidance works without touch and provides clear instructions, timers, and voice or gesture controls. This allows users to advance or repeat steps smoothly even with wet or greasy hands.

AI supports the product, but we set clear boundaries. To reduce hallucinations, the system is grounded in a curated recipe database that we clean and normalize into a structured corpus for search and recommendations. For LLM-generated recipes, we collect user feedback and ratings to guide iterative improvements. We also state that we do not train proprietary models, we avoid maintenance-heavy inventory management, and we do not provide medical-grade dietary advice. Given model invocation cost and token limits, we also optimize for reliability within limited context.

From a systems perspective, we adopt a cross-platform architecture to reduce development effort while maintaining a consistent user experience. The mobile client is built with React Native, the backend uses C\# and .NET, and PostgreSQL serves as the core data store.
...
Following a period of intensive development, the project's functionalities have been fully implemented. The ``Discover'' interface on the home page has successfully deployed a two-tower model algorithm. This algorithm comprehensively processes user profiles, historical browsing trajectories, and latent preferences to deliver precise, personalized recipe recommendations. The accompanying diet-planning feature has also reached maturity; not only can users utilize it to schedule future meals, but the system is also capable of proactively sending notifications at preset times.

In the AI domain, we have achieved a significant evolution from simple dialogue capabilities to a full-fledged Agent system. The integrated AI module now supports the Model Context Protocol (MCP), enabling it to transcend the role of a mere chatbot for retrieving or generating recipes. It now possesses the capability to operate internal application functions, allowing it to act as an agent on behalf of the user to manage diet plans or modify shopping lists. Notably, our model possesses multimodal capabilities, allowing it to leverage image recognition for various functions. For instance, users need only capture a photo of their refrigerator's interior; the system identifies existing inventory, compares it against target recipes, automatically filters out overlapping items, and adds missing ingredients to the shopping list, thereby greatly simplifying the restocking process.

To address practical pain points within the cooking environment, we have also refined the interactive experience. The step-by-step cooking instruction page now supports ``Always On Display'' to prevent screen timeouts from interrupting the cooking flow, and a timer widget has been directly integrated into the interface. Considering that users' hands may be wet or oily while cooking, we have expanded upon our existing air gesture recognition by adapting functionality for the Apple Watch. This allows users to control step transitions via the ``Double Tap'' gesture---pinching the index finger and thumb together. Furthermore, we have conducted customized development tailored to specific hardware platform characteristics, including support for Dynamic Island and Home Screen Widgets on iOS, as well as utilizing sensors on specific flagship Android devices to implement temperature measurement functionality.

At the infrastructure level, we have completed the construction of the user login and authentication system. We also performed deep cleaning of the underlying database, achieving standardized unification of all ingredient measurement units and consistent naming conventions for ingredient categories. To provide a superior user experience, the application now fully supports Dark Mode and multi-language switching, ensuring the system's usability across diverse environments and user demographics.
